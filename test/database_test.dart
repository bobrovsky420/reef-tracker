...
void main() {
  ...
  group('createTankWithPreset', () {
    test('seeds the preset parameters in catalog order with preset bounds',
        () async {
      final id =
          await db.createTankWithPreset(name: 'Reef', type: SetupType.sps);
      final params = await db.getTrackedParameters(id);

      expect(params.map((p) => p.paramKey).toList(),
    ...
    test('every setup type seeds exactly its preset keys', () async {
      for (final type in SetupType.values) {
        ...
        expect(keys, unorderedEquals(defaultTrackedKeys(type)),
  ...
  group('foreign-key cascade', () {
    test('deleting a tank removes its readings and actions', () async {
      ...
      await db.insertWaterChange(
          tankId: id, changedAt: DateTime(2026, 1, 2), amountLiters: 20);
    ...
    test('deleting the active tank reassigns active to a remaining tank',
        () async {
  ...
  group('reading groups', () {
    ...
    test('updateReadingGroupTime moves a whole group', () async {
      ...
      await db.insertReadingGroup(tankId: id, takenAt: from, values: const [
        (paramKey: 'ph', value: 8.1),
        (paramKey: 'alkalinity', value: 8.5),
      ]);
  ...
  group('row -> domain bridges', () {
    test('boundsOf mirrors the stored zone bounds', () async {
      final id =
          await db.createTankWithPreset(name: 'A', type: SetupType.sps);
      final alk = (await db.getTrackedParameters(id))
    ...
    test('presentationOf converts temperature to the preferred unit', () async {
      final id =
          await db.createTankWithPreset(name: 'A', type: SetupType.mixed);
      final temp = (await db.getTrackedParameters(id))
          .firstWhere((p) => p.paramKey == 'temperature');
      final pres =
          presentationOf(temp, const UnitPrefs(temp: TempUnit.fahrenheit));
