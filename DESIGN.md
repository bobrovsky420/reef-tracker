...
- **Android-first**, iOS port in progress: `ios/` was regenerated 2026-07-07
  ...
  code path ever requests these permissions, so the strings are never shown). Launch screen
...
- **iOS builds require macOS** — nothing under `ios/` can be compiled on this
  ...
  a manually-triggered `ios-release` workflow (mac_mini_m2, free personal tier)
  that builds a signed IPA and uploads it to TestFlight. Signing is automatic —
  Codemagic manages the distribution certificate/profile via an App Store
  Connect API key added in its UI (integration name `ReefTracker ASC key`).
  The CocoaPods `Podfile` is generated by the Flutter tool on first macOS build
  (commit it back once it appears).
