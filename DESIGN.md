...
- Known iOS gap: `notifications.dart` is Android-only (no Darwin init settings /
  permission request / notification details yet) — reminders silently no-op on iOS
  until the Darwin branch is added.
