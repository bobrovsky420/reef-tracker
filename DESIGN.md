...
- **Shipped on both stores** from this one codebase — Android on Google Play,
  iOS on the App Store (first 0.x release approved 2026-07). `ios/` was
  regenerated 2026-07-07 (`flutter create --platforms=ios --org cz.reeftracker`)
  with bundle id `cz.reeftracker.reeftracker` (matches the Android application
  id), deployment target iOS 13, display name `ReefTracker`. `Info.plist`
  declares `CFBundleLocalizations` (en/cs/de/ru/pl/fr/it),
  `ITSAppUsesNonExemptEncryption=false` (no custom crypto — skips
  export-compliance questions on upload), and camera/photo-library purpose
  strings (ITMS-90683: originally required only because `file_picker`'s iOS
  code references those APIs; since the checker camera scan (U34) the camera
  permission IS requested by a real code path, so the camera string is
  user-visible and must stay accurate). Launch screen
  uses `systemBackgroundColor` (mirrors Android's `?android:colorBackground`
  light/dark splash). iOS icons are generated by `flutter_launcher_icons`
  (`ios: true`, alpha flattened onto `#DEF5F8`).
- **iOS builds require macOS** — nothing under `ios/` can be compiled on this
  Windows machine; builds run on Codemagic ([codemagic.yaml](codemagic.yaml)),
  toolchain pinned (T20: `flutter` + `xcode` in codemagic.yaml, matched by
  ci.yml — bump deliberately, together). Two manually-triggered workflows on
  mac_mini_m2 (free personal tier):
  - `ios-release` — test builds: signed IPA → TestFlight. Signing is
    automatic — Codemagic manages the distribution certificate/profile via an
    App Store Connect API key added in its UI (integration name `ReefTracker
    ASC key`).
  - `ios-store-release` — **the release path**: same build, then pushes the
    localized "What's New" for every language from
    `store_assets/release_notes/<version>/appstore/<locale>/release_notes.txt`
    via Codemagic's `app-store-connect` CLI. **That path is load-bearing**: the
    workflow derives it from `pubspec.yaml`'s version and fails the build if
    the directory is missing (see CLAUDE.md "Release notes"). App Store
    submission stays off until flipped (`submit_to_app_store`), promotion is
    manual.
  The CocoaPods `Podfile`/`Podfile.lock` only materialize on macOS and are not
  yet committed; both workflows expose them as build artifacts so the pair from
  the next green build can be committed, locking the transitive pod set (T20).
- **iPhone-only** (`TARGETED_DEVICE_FAMILY = 1`): keeps v1 simple — no iPad
  screenshots required in App Store Connect, and `share_plus` needs no
  `sharePositionOrigin` (that's an iPad-popover requirement).
- `notifications.dart` has full Darwin support (lazy permission request,
  `threadIdentifier` grouping, 64-pending cap — see the Reminders section);
  `AppDelegate.swift` wires the `UNUserNotificationCenter` delegate.
- **Offline-first, single-user, no backend and no account.** All data lives in
  the local SQLite database. The app does make network calls, but only for
  user-enabled features: cloud backup sync to the user's own Drive/iCloud
  (U24/U44), LAN reads from aquarium devices on the home network (U36/U38/U39/
  U40), BLE to the Hanna checker (U33/HANNA.md), and external website links.
  Nothing talks to a ReefTracker server — there is none.
...
surfacing: both/badge/off), `stability_window` (days of history the stability
score examines, U26: 30/60/90, default 30 — the longer windows let relaxed
testing cadences accumulate enough readings for a score),
`tour_v1_seen` (first-run feature-tour flag),
...
Notable DB behavior:
- `createTankWithPreset` seeds *which* parameters a tank tracks from the
  setup-type preset and makes the tank active, all in one transaction. It
  deliberately does not copy their bounds or targets — see below.
- **Bounds and targets are resolved on read, never seeded** (v28).
  `ResolvedParameter.resolve(row, setupType, override)` — and
  `resolveParameters` for a whole tank — answers with the tank's
  `ParameterOverrides` row where one exists and `defaultBoundsFor` /
  `defaultTargetFor` where it does not. Consequences worth knowing: changing a
  tank's setup type instantly re-colours every parameter it has not
  overridden, shipping new catalog defaults reaches every existing install
  with no migration, and there is nothing to "re-apply".
  `ResolvedParameter` is what the UI and the domain aggregates consume; it
  **cannot be written back to the database** (only its `.row` can, and `.row`
  never carries resolved values), which is what makes it impossible for a
  wholesale row write — the enable/disable toggle does one — to silently
  freeze today's defaults into a tank.
- `boundsOfOverride(ParameterOverride)` builds `ZoneBounds` from a stored
  override; `presentationOf(ResolvedParameter)` / `presentationOfRow(
  TrackedParameter)` bridge to a `ParamPresentation`.
- `setParameterOverride` / `clearParameterOverride` write and drop a single
  parameter's override; `resetParameterDefaults(tankId)` drops all of a tank's
  ("Reset all parameters to defaults" in the app-bar menu). None of them
  recompute anything — deleting the row *is* the reset.
...
**dirty gate**: sha256 of the document with `exportedAt`, `device` and
`checksum` stripped and the `settings` section **normalized** (#93) —
device-local keys re-filtered (pre-#69 cloud files still carry them), the
sticky `legacy_free_since` dropped (a restore plants it locally), remaining
rows sorted by key (the table accretes rows in per-device first-write order).
Decode–strip–re-encode, so two backups of the same aquarium data hash
identically even though every encode stamps a fresh `exportedAt`. The
settings section *is* hashed because what survives the #69 filter is
data-bearing (`hanna_method_sets`, `device_vendor_order`, `ro_stages_seeded`,
…) — stripping it wholesale made settings-only edits invisible to sync and
silently overwritable by a U35 fast-forward restore. Equal hash ⇒ nothing to
upload (see Google Drive sync below).
...
new files as before; `backupContentHash` applies the same device-local filter
before hashing (#93), so the U24 dirty gate and U35 lineage checks are
unaffected. The U19 founder marker `legacy_free_since` is deliberately **not**
...
both manual export and the automatic backup service. All the table reads run
in **one drift transaction** (#92), so the document is a consistent snapshot —
a write landing mid-encode (or a concurrent `importBackup`) queues behind it
instead of tearing the sections apart into a file `validateBackup` would
reject at restore time. The CPU-heavy halves of
...
- **`notifications.dart` — the platform wrapper.** Thin service over
  `flutter_local_notifications`: init + tap callback, the platform permission
  (Android 13+ POST_NOTIFICATIONS / iOS alert+badge+sound authorization —
  asked when the user first enables a category, never at startup; the Darwin
  init settings keep all `request*Permission` flags off for the same reason),
  three notification channels (one per `ReminderKind`, so system settings can
  tune each; iOS has no channels — the same ids become Darwin
  `threadIdentifier`s, grouping reminders of a kind in Notification Center),
  and `syncPlanned(list)` = `cancelAll()` + schedule fresh (safe: the app owns
  no other notifications and every sync recomputes the full set). The sync
  sorts by fire time and schedules **at most the 64 soonest** on every
  platform — iOS silently drops pending requests beyond 64, and the frequent
  resyncs refill the window long before it drains (tested in
  `test/notifications_sync_test.dart` via a fake plugin). Scheduling is
  **inexact** (`AndroidScheduleMode.inexactAllowWhileIdle` — reminders don't
  need minute precision, and it avoids the SCHEDULE_EXACT_ALARM
  permission/Play review; observed delivery windows are minutes). Instants are
  scheduled as **absolute UTC** (`tz.TZDateTime.from(utc, tz.UTC)`), so no
  timezone-lookup plugin is needed; a DST shift is at most an hour off until
  the next resync. The `ReminderSink` interface is the seam scheduler tests
  fake. The app manifest declares the plugin's `ScheduledNotificationReceiver`
  + `ScheduledNotificationBootReceiver` (with RECEIVE_BOOT_COMPLETED), which
  the plugin does **not** declare itself — reminders would otherwise silently
  not fire / not survive reboots. On iOS, `AppDelegate.swift` sets the
  `UNUserNotificationCenter` delegate so foreground presentation and taps
  reach the plugin.
...
### Diagnostics log (#107) — `data/diagnostics_log.dart`

The offline answer to crash reporting (there is no backend, so no Crashlytics
class of service): `installDiagnosticsHooks` in `main()` wraps
`FlutterError.onError` (everything funneled through
`FlutterError.reportError` — the observer above, the fire-and-forget launch
tasks, the framework itself — still reaches the previous handler for the
console) and sets `PlatformDispatcher.instance.onError`, so unhandled async
errors join the same funnel instead of vanishing in release. Entries land in
`DiagnosticsLog`, a best-effort ring-buffer file under the app documents dir
(`diagnostics.log`, rotated once past 64 KB with one previous generation
kept, ~128 KB worst case; writes serialized on one future chain; a
per-process record cap stops an error storm from looping the ring; the
documents-dir lookup is bounded + memoized-on-success-only against the
pre-first-frame platform-channel hang, flutter/flutter#72872). Settings →
About → **Share diagnostics** shares the concatenated log as plain text via
the standard `export_share.dart` staging/sweep path
(`kDiagnosticsExportPrefix`), headed by app version + OS; an empty log gets a
SnackBar instead of an empty file. Nothing is ever sent anywhere by itself.
Tests: `test/diagnostics_log_test.dart` (round-trip, rotation, broken-dir
recovery, both hooks, record cap).
...
- `tankStabilityProvider` — `TankStability` for the active tank (U26, Pro).
  ...
  (a private autoDispose family over `watchReadingsSince`, keyed by
  (tank, window) — a *time* window rather than `kRecentReadingsPerParam`'s
  count cap, which would silently truncate a daily tester's 60/90-day series;
  the cutoff fixed at stream creation only ever makes the feed a superset,
  and `computeTankStability` re-filters with its own clock), feeding each core
...
frame. For a **dosable element** (`kDosingElementKeys`) the AppBar also carries a
calculator icon opening the dose calculator on this element
(`/dosing/calculator?element=<key>`, Pro-gated with the same teaser dialog as
the Dosing tab's tile); when the latest reading in range additionally sits
**below the green zone**, an inline `_CorrectionCta` card under the trend card
offers the calculator's correction mode directly (`&mode=correction`) — only
below, since a correction dose can only raise (too-high is water-change
territory). A **quick-add FAB** (U30) records a fresh reading for the viewed
...
An **oscillating** parameter (#31) gets no forecast either, for the opposite
reason: its fitted slope failed the significance gate, so there is no trusted
direction to project. Both widgets swap the direction arrow for `swap_vert` —
the card replaces the per-day rate headline with the measured swing
(`±σ` in display units, the affine delta conversion again) over a "Swinging —
no clear direction" note; the dashboard chip shows a neutral, deliberately
un-alarming "Unsettled" in `textDim` rather than a zone colour, and is **not**
horizon-gated (there is no crossing date to compare against). A non-significant
slope whose swing is *below* `kTrendOscillationRelative` isn't oscillating —
the card falls back to "Holding steady" (not "staying within range at this
rate", which would imply a rate worth trusting) and the chip renders nothing.
...
- **Maintenance schedule (U12), elastic + planned.** `/schedule` (calendar
  ...
  `lastDoneAt`. Because that anchor predates the plan itself, an explicit
  first-due date (`scheduledAt`) **floors** the computed due date in every
  repeat mode — and a one-off is only retired by a completion at/after its
  planned date, never by a pre-plan action (#97). The Actions tab shows a horizontally scrollable **due-chip
...
Tracking of the ICP element panel (33 elements — modeled on the Fauna Marin
Reef ICP: 6 major ions incl. silicon, 14 desirable traces, 13 contaminants),
...
- **`MicroScreen`** (`/micro`): summary header card (dominant-zone icon +
  ...
  `/history/:paramKey`; zone bounds are edited from the **element settings**
  action (see `MicroConfigureScreen` below), not per row. The app bar keeps
  **ICP import** as its one visible button; manage views, element settings
  and the test reminder sit in a ⋮ overflow `ReefMenuButton` (title room).
  **Tracked rows are lazy** (U17): created only when the user saves a value
  ...
  zero rows. An overflow-menu action creates a **test reminder** as a plain custom
  `MaintenanceSchedules` task (default every 90 d) — reusing the whole
  U12 reminder machinery, no parallel path.
- **`MicroConfigureScreen`** (`/micro/configure`, the "Element
  settings" overflow action): every catalog element in report-section order with its
  *effective* bounds as subtitle; tapping opens the standard
  `ParameterEditScreen`, creating the element's tracked row on demand (same
  lazy-row rule). Deliberately unfiltered by the active view — bounds are
  configurable for any element, not just the ones the current lab reports.
...
- Values are entered and displayed in the **units printed on the ICP report**
...
- **Element views** (U17): a chip row on the screen — `[Full list]
  ...
  *frozen explicit key list* — deliberately not derived from the catalog,
  ...
  view's elements — or the manage sheet in the ⋮ overflow; chip long-press is the
...
badge everywhere the feature surfaces (the Devices-page checker card, screen
title, in-screen
...
- **Platform:** `flutter_blue_plus` pinned `<2.0.0` (the 2.x line requires a
  ...
  entry points (the Devices-page checker card's measurement action and the
  overflow menu entry) hide; the connect screen's "unsupported"
...
There is no separate device inventory screen: the read-only Settings →
Connected devices union (`/settings/devices`) was removed by U41 — the unified
...
- **Shape** (`wall_screen.dart`): a pushed scaffold-less route — no app bar,
  ...
  "days at the current rate" is stale), the supplement's abbreviation
  (`shortName ?? supplement`; unnamed sockets skipped) and the time the
  container lasts, compacted by `wallSupplementTimeLeft` (precise days under
  100 — every amber/red state lives there — rounded 30-day months beyond);
...
- **Manual doses** (`manual_dose_edit_screen.dart`, route `/dosing/manual`):
  ...
  now). The route's `extra` may alternatively carry a `ManualDoseDraft`
  (product/element/amount/unit prefill for a *new* dose — the dose
  calculator's "log this dose" handoff) instead of a `ManualDose` to edit. Timeline
...
`_index == 2`) and from a dosable parameter's **history screen** (app-bar
calculator icon + the below-range correction CTA — both pass
...
element back). **Correction** computes the one-off dose that raises the
element from its current value to a target. The math is the pure
`domain/dose_calculator.dart`; the screen is
...
- **Correction mode.** Shares the element, volume and potency sections; the
  window/current-dose/manual-dose inputs are replaced by **current value** and
  **target value** fields (canonical units, like the rest of the screen). Both
  ...
  Lowering is out of scope — at/above target reports "nothing to dose". A
  **"Log this dose"** button (when a dose was computed) pushes
  `/dosing/manual` with a `ManualDoseDraft` prefill (element, the **per-day**
  portion for a split — what was actually given today — the plan's dose unit,
  and the product key only when the dose was computed from its catalog
  strength), closing the loop: logged corrections feed back into the
  maintenance math's logged-doses default.
...
`ParameterEditScreen`'s Safe ranges card additionally carries the **correction
target** field (`ParameterOverrides.targetValue`, edited in the same display
space as the bounds; empty = the setup-type preset's target, then the
green-zone midpoint at use time) — the value the dose calculator's correction
mode pre-fills.

The editor is always seeded with the parameter's *effective* values, whether
those come from an override or the defaults, so the user edits the numbers
they can see. Saving stores an override — **unless** the submitted bounds and
target still equal the resolved defaults, in which case any existing override
is deleted instead. Without that rule, opening this screen to change only the
test cadence would silently opt the parameter out of every future default
change. A parameter that carries an override shows a **Reset to defaults**
action (deletes just its row); one that does not shows a "Following the
...
- Regenerate the parameter catalog after editing `lib/domain/parameters.yaml`:
  `dart run tool/gen_parameters.dart` (validates + writes
  `parameter_catalog.g.dart`).
...
- Regenerate the Pro feature registry after editing `lib/domain/pro_features.yaml`:
  ...
  `pro_features.g.dart`).
- Regenerate the tank presets after editing `lib/domain/tank_presets.yaml`:
...
- Tests (`flutter test`): domain — `test/zones_test.dart`,
  ...
  weighting, staleness), `test/stability_score_test.dart` (detrending,
  windowing, span guard, the worst-parameter cap),
  `test/insights_test.dart` (the four insight rules, ordering, side
  placement; `test/insights_card_test.dart` covers the card's Pro gate),
  `test/zone_bands_test.dart` (chart band geometry);
...
- **CI (`.github/workflows/ci.yml`):** every push/PR to `master` runs two jobs
  on `ubuntu-latest` (Flutter pinned to 3.44.6, matching the local toolchain
  and the codemagic.yaml pins — T20; superseded runs on the same ref
...
### Build junctions & the build scripts (`scripts/`)

The repo originally lived inside OneDrive, which locks Flutter's churning
build files; it moved out (to `C:\Dev\ReefTracker`) on 2026-07-02, but the
volatile directories (`build`, `.dart_tool`, `android/.gradle`,
`android/.kotlin`) remain **NTFS junctions** pointing at
`C:\Android\reefbuild\ReefTracker\` (override with the `REEFBUILD_ROOT` env
var) — the redirect still helps (Defender/indexing churn, stable out-of-tree
artifact paths) and the scripts assume it.

- **Never run `flutter clean` here** — it deletes the junctions, and the next
  build recreates them as real in-repo folders, so release artifacts land in
  a stale location. Use
