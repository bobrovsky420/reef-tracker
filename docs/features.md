...
| Hanna checker direct connection *(experimental)* — run measurements live over Bluetooth on a Hanna HI97115C photometer, method by method or via saved test sets, and save the confirmed results with the meter's timestamps. Built-in reagent reaction timers (15 s / 30 s / 2 min) beep when the wait is over, and the screen stays awake while a test is running. Measurements only: meter settings and firmware updates stay in Hanna's own Hanna Lab app | **Pro** |
